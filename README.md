...
* **다양한 노드 타입**: 메시지, 폼, 조건 분기, 슬롯 설정, 외부 API 호출, LLM 연동 등 챗봇 시나리오에 필요한 다양한 기능을 노드 형태로 제공합니다.
...
| **Condition Branch** | 사용자의 답변이나 슬롯 값에 따라 대화 흐름을 여러 갈래로 나눕니다. |
...
| **Set Slot** | 하나 이상의 슬롯(변수) 값을 직접 설정하거나 업데이트합니다. 흐름 제어나 값 초기화에 사용됩니다. |
...
| **iFrame** | 외부 웹페이지를 챗봇 대화창 내에 직접 삽입(embed)합니다. |
...
-   **수정/복제/삭제**: 각 항목 옆의 아이콘을 통해 이름 변경, 시나리오 복제, 영구 삭제를 할 수 있습니다.
